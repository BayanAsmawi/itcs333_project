document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("reviews.json")
    ...
    .then((data) => {
      ...
      document.getElementById(
        "total-reviews"
      ).textContent = `Based on ${totalReviews} reviews`;

      for (let rating in reviewCounts) {
        const percentage = (reviewCounts[rating] / totalReviews) * 100;
        document.querySelector(
          `.progress-bar[data-star="${rating}"]`
  ...
  function generateStars(rating) {
    let starsHtml = "";
    for (let i = 1; i <= 5; i++) {
      starsHtml += `<i class="bi bi-star-fill star ${
        i <= rating ? "fullStar" : ""
      } fs-2"></i>`;
    }
    return starsHtml;
...
async function fetchData() {
  try {
    ...
    document.getElementById("cards-container").innerHTML =
      "<p>Some thing went wrong while loading Data !</p>";
  }
}
